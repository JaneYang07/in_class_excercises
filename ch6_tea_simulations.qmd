...
```{r}
by_condition <- tea_data |> group_by(condition) |> summarize(mean_rating = mean(rating),num_participants=length(rating))


# OR: by_condition_v2 <- tea_data |> summarize(mean_rating = mean(rating),.by="Condition")
```
...
```{r}
by_condition_highn <- tea_data_highn |> group_by(condition) |> summarize(mean_rating = mean(rating),num_participants=length(rating))
...
# samps <- tibble(sim = 1:1000) |> # 
#   mutate(data = map(sim, \(i) make_tea_data(n_total = 18, delta=2))) |>  # simulate
#   unnest(cols = data) # wrangle
```
...
# samps_highn <- tibble(sim = 1:1000) |> # 
#   mutate(data = map(sim, \(i) make_tea_data(n_total = 48, delta=2))) |>  # simulate
#   unnest(cols = data) # wrangle
```
...
```         
## mean is convering to the new delta, and greater sigma will result in a greater variance
```
...
ggplot(data=null_model_summary, aes(x=delta)) +
  geom_histogram(alpha=.8, bins=20) +  theme_few()
```
...
```{r}
false_pos = sum(all_results$pvalue <= 0.05)/nrow(all_results)
false_pos
```
...
```{r}
sum(all_results_high_n$pvalue>.05)/nrow(all_results_high_n)
```
